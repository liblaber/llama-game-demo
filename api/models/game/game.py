...
class MoveResult(BaseModel):
    ...
    status: LlamaStatus = Field(
        default=LlamaStatus.ALIVE, description="Current Llama's status."
    )
    position: List[int]
...
class StepResult(BaseModel):
    ...
    direction: Direction = Field(
        default=Direction.RIGHT, description="The direction the llama will move in."
    )
    steps: int = Field(
        default=0, description="The amount of steps the llama will take."
    )

    model_config = {
        "json_schema_extra": {
            "examples": [{"llama_id": 123, "direction": "right", "steps": 5}]
        },
